    ...
    ease: [0.4, 0, 0.2, 1]
      ...
      <nav className={`container-custom mx-auto flex items-center justify-between rounded-3xl px-6 py-3 transition-all duration-300 border border-foreground/20 ${isScrolled ? 'bg-background/95 backdrop-blur-md shadow-lg' : 'bg-background/80 backdrop-blur-sm'}`}>
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map(link => <a key={link.name} href={link.href} onClick={() => handleLinkClick(link.name)} className={`relative text-sm font-medium transition-colors duration-300 ${activeLink === link.name ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
              {link.name}
